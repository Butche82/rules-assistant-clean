...
export default function Page() {
  ...
  const loadGames = async () => {
    try { const data = await api("/api/games"); setGames(data.games || []); }
    catch { setGames([]); }
  };
  useEffect(() => { loadGames(); }, []);
  ...
  const ask = async () => {
    ...
    setChat(c => [...c, userTurn]); setQ(""); setLoadingAnswer(true);
    try {
      ...
      const assistantTurn: ChatTurn = { role: "assistant", content: data.answer || "(No answer returned)", citations: data.citations || [] };
      setChat(c => [...c, assistantTurn]);
    } catch (e:any) {
      setChat(c => [...c, { role: "assistant", content: "Error contacting /api/chat: " + (e?.message || e) }]);
      ...
      <div className="mx-auto max-w-7xl grid gap-6 lg:grid-cols-[340px_1fr] xl:grid-cols-[380px_1fr]">
        ...
        <div className={`space-y-6 ${showPanel ? "block" : "hidden"} lg:block`}>
          ...
          <div className="rounded-2xl border bg-white shadow-sm">
            ...
            <div className="p-4 grid gap-2">
              {games.length===0 && (<div className="text-sm text-slate-500">No games indexed yet. Hit <strong>Sync</strong> above.</div>)}
              {games.map(g=>{
        ...
        <div className="rounded-2xl border bg-white shadow-sm flex flex-col h-[calc(100vh-160px)] sm:h-[calc(100vh-140px)]">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {chat.length===0 && (<div className="text-slate-500 text-sm">Try: <em>"In Ticket to Ride Europe, can I build a station after someone blocks a route?"</em></div>)}
            {chat.map((t, idx)=>(
            ...
            {loadingAnswer && (<div className="flex items-center gap-2 text-slate-500 text-sm"><Loader2 className="h-4 w-4 animate-spin"/> Thinking…</div>)}
            <div ref={endRef}/>
          </div>
          ...
          <div className="p-4 border-t bg-white sticky bottom-0">
            <div className="mb-2 flex flex-wrap items-center gap-2 text-sm">
              ...
              <button onClick={()=>{setStrictMode(true); setAllowInterp(false);}} className={`px-3 py-1.5 rounded-full border ${strictMode && !allowInterp ? "bg-slate-900 text-white border-slate-900" : "bg-white"}`} title="Only answer when a citation is available">Strict (citations only)</button>
              <button onClick={()=>{setStrictMode(false); setAllowInterp(true);}} className={`px-3 py-1.5 rounded-full border ${!strictMode && allowInterp ? "bg-slate-900 text-white border-slate-900" : "bg-white"}`} title="Permit advisory interpretations for ambiguous cases">Allow interpretations</button>
            </div>
